...
void QCPPDialogImpl::readSettings()
{
   ...
   if (!entryFound)
   {
      devices<<"/dev/ttyS0"<<"/dev/ttyS1"<<"/dev/ttyUSB0"<<"/dev/ttyUSB1";
   }
...
{
   QMessageBox::about(this, tr("About CuteCom"), tr("This is CuteCom 0.22.0<br>(c)2004-2009 Alexander Neundorf, &lt;neundorf@kde.org&gt;<br>Licensed under the GNU GPL v2<br><br>Modified for BionX BIB Comm<br>by --Automated--<br>This adds CRC16 and EOT-Bit<br>in HEX Mode"));
}
...
bool QCPPDialogImpl::sendString(const QString& s)
{
   ...
   if (lineMode==4) // hex
   {
      ...
      for (int i=0; i<hex.length()/2; i++)
      {
         ...
         input[inputlen] = (unsigned char)byte;
         inputlen++;
         //fprintf(stderr, "\\x%02x", byte);//Ausgabe: \x01\xf1\xd1\xf8
         //fprintf(stderr, "\%02x", byte);//OK, gibt nur Hex Werte aus
         //fprintf(stderr,"d:%d", byte & 0xff, charDelay);
      ...
      fprintf(stderr, "\n");
*/
      unsigned short crc = crc16(input,inputlen);//(const unsigned char* data_p, unsigned char length)
/*
      fprintf(stderr, "CRC16: ");
      fprintf(stderr, "\%02x", crc);
      fprintf(stderr, "\n");
*/
      unsigned char crcH = crc >> 8;
      crc = crc << 8;
      unsigned char crcL = crc >> 8;
      /*
      fprintf(stderr, "crcH: ");
      fprintf(stderr, "\%02x", crcH);
      fprintf(stderr, ", crcL: ");
      fprintf(stderr, "\%02x", crcL);
      fprintf(stderr, "\n");
      */

      unsigned char crcplusend[3] = {0x01,0x02,0xF8};
      crcplusend[0] = crcH;
      crcplusend[1] = crcL;
      /*
      fprintf(stderr, "TEST: ");
      fprintf(stderr, "\%02x", crcplusend[0]);
      fprintf(stderr, "\%02x", crcplusend[1]);
      fprintf(stderr, "\%02x", crcplusend[2]);
      fprintf(stderr, "\n");
      */
      ...
      /*
      fprintf(stderr, "len: ");
      fprintf(stderr, "\%02x", len);
      fprintf(stderr, "\n");
      */
      unsigned int byte = 0;
      for (int i=0; i<len; i++)
      {
         if (i < (len-3))//Send the HEX Bytes from Input
         {
            QString nextByte=hex.mid(i*2, 2);
         ...
         {
            int ptr = i -len +3;//Send CRC16 and EOT
            byte = crcplusend[ptr];
            sendByte(byte & 0xff, charDelay);
         }
...
//      std::cerr<<"readData() "<<bytesRead<<std::endl;
   ...
   QString text;
   //#define buflen 16
   char buf[bytesRead+3];
   //addOutput("TEST");
   int buflen = bytesRead;
   ...
   for (int i=0; i<bytesRead; i++)
   {
      if (m_hexOutputCb->isChecked())
      {
         ...
         unsigned int b=*c;
         snprintf(buf, buflen, "%02x ", b & 0xff);
         text+=buf;

         m_hexBytes++;
         /*
         if ((m_hexBytes % buflen)==0)
         {
            text+="\n";
         }
         */
      ...
      if (m_hexOutputCb->isChecked())
      {
         if ((m_hexBytes % buflen) == 0)
         {
            snprintf(buf, buflen, "%08x: ", m_hexBytes);
            text+=buf;
         }
         unsigned int b=*c;
         snprintf(buf, buflen, "%02x ", b & 0xff);
   ...
   if (m_hexOutputCb->isChecked())//also print the received Data as ASCII
      {
         text+="  ASCII:";
         const char* d=m_buf;
         for (int i=0; i<bytesRead; i++)
